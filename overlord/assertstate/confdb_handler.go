...
func (c *valsetsConfdbHandler) Commit(st *state.State, tx *confdbstate.Transaction) ([]*state.TaskSet, error) {
	...
	for _, path := range tx.AlteredPaths() {
		...
		// Databag will need changes if we add v2 paths in the confdb-schema, so fail
		// here to flag the issue
		if path[0].Name() != "v1" {
...
// applyChanges uses the values set through confdb to update the
// ValidationSetTracking.
func applyChanges(accountID, name string, tr *ValidationSetTracking, val any) error {
